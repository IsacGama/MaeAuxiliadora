...
export default function AccountSetupScreen() {
  ...
  useEffect(() => {
    ...
    const loadPreview = async () => {
      if (!tokenValue) {
        setPreview(null);
        setPreviewError('Link de ativação inválido ou ausente.');
        setIsLoading(false);
        return;
      }
      ...
      try {
        const result = await authApi.previewAccountSetup(tokenValue);
        if (cancelled) return;
        setPreview(result);
  ...
  const onComplete = async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : 'Não foi possível ativar a conta.';
      setError(message);
    } finally {
      setSubmitting(false);
    }
